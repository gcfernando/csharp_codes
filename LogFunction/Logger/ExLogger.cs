...
public static class ExLogger
{
    ...
    public static void Log(ILogger logger, LogLevel level, string message, Exception exception, params object[] args)
    {
        ...
        if (_delegates.TryGetValue(level, out var del) && (args == null || args.Length == 0))
        {
            // Fast path (no object[] allocation, avoids boxing)
            del(logger, message, exception);
        }
        else
        {
            // Structured logging fallback (slower, but supports args)
